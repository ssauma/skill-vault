...
Skill Vault는 OpenAI Codex와 Claude Code를 위한 오픈소스 CLI이자 Agent Skill 컨텍스트 관리자입니다. 스킬 탐색 메타데이터와 컨텍스트 윈도우 사용량을 진단하고, Codex에서는 사용자가 승인한 독립형 스킬만 완전히 롤백 가능한 명시적 온디맨드 로딩 프록시 뒤로 격리합니다. 기존 `$skill-name` 호출 방식은 바뀌지 않습니다.
...
- 설치된 Agent Skill의 탐색 메타데이터와 컨텍스트 부담 진단
- 기존 직접 호출을 유지하면서 승인된 지침만 필요할 때 로드
- 원본 스킬을 수정하지 않고 소유한 프록시와 설정 변경을 완전히 롤백
